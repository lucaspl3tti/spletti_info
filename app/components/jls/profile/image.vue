...
import type { JlsProfileImageProperties } from '@/interfaces/content/profile.interface';

const properties = withDefaults(defineProps<JlsProfileImageProperties>(), {
  data: null,
});
...
const hasSocialLink = computed(() => socialLinkUrl.value && socialLinkUrl.value !== '');
const hasSocialLinkLabel = computed(() => socialLinkLabel.value && socialLinkLabel.value !== '');
</script>
...
@include tablet-up {
  .jls-profile-image {
    ...
    .speech-bubble {
      @include position($top: 58%, $left: 30%);
      @include flex;
      place-items: center;
      opacity: 0;
      min-width: 40px;
      ...
      &::before {
        content: '';
        @include position($top: -12%, $left: 1.5rem);
        @include size(0);
        border: 0.75rem solid transparent;
        border-top: none;
        border-bottom-color: $color-white;
        filter: drop-shadow(0 -0.0625rem 0.0625rem rgba($color-black, $alpha: 0.1));
      }
    }
  ...
  @include light-mode {
    .jls-profile-image {
      &.has-social-link {
        &:hover {
          .jls-profile-image__img {
            box-shadow: 20px 20px 18px rgba($color: $color-primary-300, $alpha: 0.6);
          }
        }
      }
    }
