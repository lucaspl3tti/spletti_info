<template>
  <div
    :class="[
      'clb-loader-dots',
      `clb-loader-dots--${theme}`,
      `clb-loader-dots--${size}`,
      `dots-${variant}`,
    ]"
  >
    <div v-if="variant === 'growing'" class="clb-loader-dots__wrapper">
      <div
        v-for="number in 3"
        :key="`clb-loader-dots-item-${number}`"
        class="clb-loader-dots__circle"
      >
        <div class="clb-loader-dots__circle-dot"></div>
        <div class="clb-loader-dots__circle-outline"></div>
      </div>
    </div>

    <div
      v-else-if="variant === 'spinning'"
      class="clb-loader-dots__wrapper"
    >
      <div
        v-for="number in 8"
        :key="`clb-loader-dots-item-${number}`"
        class="clb-loader-dots__item"
      />
    </div>
    ...
    >
      ...
      <div
        v-for="number in 3"
        :key="`clb-loader-dots-shadow-${number}`"
        class="clb-loader-dots__shadow-item"
      />
    </div>
  </div>
</template>
...
import type { ClbLoaderDotsProperties } from '@/interfaces/components/loader.interface';
import {
  loaderDotsVariants,
  sizesExtra,
  colorVariants,
...
const properties = withDefaults(defineProps<ClbLoaderDotsProperties>(), {
  variant: 'flashing',
  theme: 'light',
  size: 'md',
});
...
checkComponentPropertyValidity(properties.theme, 'theme', 'loader-dots', true, colorVariants);
checkComponentPropertyValidity(properties.size, 'size', 'loader-dots', true, sizesExtra);
...
.clb-loader-dots {
  ...
  &--dark {
    --dots-color: #{color.adjust($color-black, $lightness: 15%)};
  }
  ...
  &.dots-growing {
    ...
    .clb-loader-dots__wrapper {
      @include flex;

      .clb-loader-dots__circle {
        position: relative;
  ...
  &.dots-typing::after {
    content: '';
    @include position($top: 0, $left: auto);
