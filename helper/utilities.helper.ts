...
export default class Utilities {
  ...
  static checkScreenSizeDown(breakpoint: string): boolean {
    const matchResult = window.matchMedia(`(max-width: ${breakpoint})`)
    return matchResult.matches
  }

  static checkScreenSizeUp(breakpoint: string): boolean {
    const matchResult = window.matchMedia(`(min-width: ${breakpoint})`)
    return matchResult.matches
  }
}
