...
import type { ApiResponse, AboutMe } from '@/interfaces/api.interface';
...
function handleAboutData(response: ApiResponse): void {
  aboutData.value = response.data as AboutMe;
}
</script>
...
.introduction {
  &__subheading {
    ...
    font-size: map.get($font-sizes-heading, 'xs', '3');
  }
...
@include tablet-up {
  .about {
    ...
    p {
      ...
      line-height: map.get($font-sizes-copy, 'large', 'md') + 4px;
    }
  }
}
